 ...
 */
public class zad3 extends JPanel {

    public static void main(String[] args) {
        JFrame window = new JFrame("Subroutine Hierarchy");
        window.setContentPane( new zad3() );
        window.pack();
    ...
    private void picture(Graphics2D g2) {
        AffineTransform saveTransform = g2.getTransform();
        Color saveColor = g2.getColor();

        g2.setTransform(saveTransform);
        g2.translate(1, 1);
        double t=0,k=(Math.PI*2)/10;
        int[] x = new int[10];
        int[] y = new int[10];
        for (int i=0;i<10;i++)
        {
            x[i]= (int) (100*Math.sin(t));
            y[i]= (int) (100*Math.cos(t));
            t=t+k;
        }
